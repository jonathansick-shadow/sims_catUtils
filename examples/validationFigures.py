...
def galaxyCountValidation(dbobj, doSaveFigs, fileBase):
    ...
    if doSaveFigs:
        ...
        ValidationUtils.plotNResidCume(compx, compy, comperr, basemean, binx,
                                       filename=fileBase+"_galaxyCountsResidCume.png")
    else:
...
def redshiftDistributionValidation(dbobj, obs_metadata, doSaveFigs, fileBase):
    ...
    if doSaveFigs:
        ValidationUtils.plotNofz(histData, zbinx, magbins, z_o_corr, z_o_err,
                                 area, filename=fileBase+"_galaxyNofz.png")
    else:
...
def ellipticityDistributionValidation(dbobj, obs_metadata, doSaveFigs, fileBase):
    ...
    for chunk, colMap in t.iter_catalog_chunks(chunk_size=10):
        ...
        for i in xrange(len(data)):
            ...
            dx, dy = diskSersic.calcDistribution(data['diskmajor'][i], data['diskminor'][
                                                 i], data['diskpa'][i], ndisk)
            bx, by = bulgeSersic.calcDistribution(data['bulgemajor'][i], data['bulgeminor'][
                                                  i], data['bulgepa'][i], nbulge)
            e1, e2, Ixx, Iyy, Ixy, r1, r_hl = SersicUtils.getEllip(
                numpy.concatenate((dx, bx)), numpy.concatenate((dy, by)))
