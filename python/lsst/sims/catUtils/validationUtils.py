...
class ValidationUtils(object):
    ...
    def calcTotalMags(cls, cat, bands):
        ...
        diskData = numpy.asarray(zip(diskfile, diskmn, diskAv), dtype=[
                                 ('sedfile', (str, 50)), ('norm', float), ('av', float)])
        bulgeData = numpy.asarray(zip(bulgefile, bulgemn, bulgeAv), dtype=[
                                  ('sedfile', (str, 50)), ('norm', float), ('av', float)])
        ...
        for ddat, bdat, adat, z in zip(diskData, bulgeData, agnData, redshift):
            dmags, matchwavelen, phiarray, dlambda = cls.calcMags(
                ddat['sedfile'], ddat['norm'], bplist, z, internalAv=ddat['av'])
            bmags, matchwavelen, phiarray, dlambda = cls.calcMags(
                bdat['sedfile'], bdat['norm'], bplist, z, internalAv=bdat['av'])
            if bmags is not None:
                amags, ased, phiarray, dlambda = cls.calcMags(
                    adat['sedfile'], adat['norm'], bplist, z, matchwavelen=matchwavelen, phiarray=phiarray, dlambda=dlambda)
    ...
    def plotNofz(cls, histData, zbinx, magbins, z_o_corr, z_o_err, area, compfunc=None, filename=None):
        ...
        for i in xrange(len(magbins[:-1])):
            ...
            plt.plot(cpts, compfunc(cpts, z_o_corr[i]+3*z_o_err[i],
                                    norm=norm), label="z$_o$+=3$\sigma$", ls="--", c='r', lw=2)
            plt.plot(cpts, compfunc(cpts, z_o_corr[i], norm=norm), label="z$_o$ = z$_o$", c='k', lw=2)
            plt.plot(cpts, compfunc(cpts, z_o_corr[i]-3*z_o_err[i], norm=norm),
                     label="z$_o$ -= 3$\sigma$", ls="-.", c='r', lw=2)
            plt.ylabel("N/deg/$3$x$10^4$s$^{-1}$km")
            plt.xlabel("redshift")
            plt.title("%i < i < %i: \n base catalog mean = %f model mean = %f" %
                      (magbins[i], magbins[i+1], meanz, meanzsamp))
            if i == 0:
